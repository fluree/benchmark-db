...
set was designed for, *without* the OpenCitations citation data. ~574 M triples.
...
(a sharded dump with per-shard `@prefix` would force a slow sequential parse).
...
- It is **not** the exact dump the SPARQLoscope paper (ISWC 2025) evaluated (that
  used DBLP dumps from `2024-04-01` to `2025-04-01`, ~390–502 M triples). Absolute
  per-query result *counts* will not match the paper's published reference yaml; the
  schema/query shapes do.
