...
| Mirror (pinned) | GitHub release [`dblp-kg-source-20260530`](https://github.com/fluree/benchmark-db/releases/tag/dblp-kg-source-20260530) (4× `.part-*`, <2 GiB each) |
...
Because the upstream URL is a rolling "latest" that changes without notice, the
exact snapshot we benchmarked is mirrored as a GitHub release. Use
`fetch-data.sh --mirror` to pull that pinned copy (reassembled from its split
parts and SHA-verified) instead of upstream.
