...
(same 105 categories). DBLP's set lives at the top level (shared by `dblp-kg` +
`dblp-core`); Wikidata-Truthy has its own under its dataset dir.

```
queries/                 the 105 DBLP .sparql files (shared by dblp-kg + dblp-core)
...
datasets/
  ...
  wikidata-truthy/       Wikidata Truthy (~8B); has its OWN query-set.tsv + queries/
