...
Datasets reported: `dblp-core` (7 engines), `dblp-kg` (Fluree vs QLever), `wikidata-truthy` (4 engines).

## Generate

```bash
python3 reporting/generate_report.py reports/<dataset>
```
...
1. Run `run_benchmark.sh --endpoint <url> [--clear-url <url>] -o results/<engine>.tsv` against it
   (see `../qlever/README.md` for QLever specifics).
