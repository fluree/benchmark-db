...
## Install (v4.0.5 release — recommended)
...
- Pin a specific version instead of `latest` by swapping the URL path, e.g.
  `.../releases/download/v4.0.5/fluree-db-cli-installer.sh`.
- Other install channels (Docker, Homebrew, Windows) are listed at
  **<https://labs.flur.ee>**.

The DBLP-core benchmark was run on the **v4.0.5 release** installed exactly this way.
(Earlier scaling/side benches used the pre-release branch
`feature/count-plan-aggregate-fastpaths` @ `ba88283`, which self-reports `4.0.4`;
that code is what became the v4.0.5 release.)
...
the binary. A source build self-reports `4.0.4`; the tagged release reports `4.0.5`.)
...
> Import note (observed on the pre-release `ba88283` build): a large **uncompressed**
> file with **high prefix cardinality** (≥256 prefixes, e.g. Wikidata) could silently
> import 0 chunks (`committed_chunks=0`). DBLP-core is low-cardinality and unaffected;
> for high-cardinality data, import the **gzipped** file. Re-verify against v4.0.5.
...
../../common/run_benchmark.sh --endpoint http://localhost:8090/v1/fluree/query/dblp:main \
  -r 3 -w 1 -t 300 -o reports/<ds>/engines/fluree.tsv
```
