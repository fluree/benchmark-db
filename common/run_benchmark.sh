...
#   --save-outputs DIR   Save each response body to DIR/<query_id>-run<N>.tsv
#                        (only successful 200 responses are saved)
#
...
# Times are written as fractional milliseconds (e.g. 0.847) with 3 decimal
# places so the geo-mean is computed on real values, not integer-truncated ones.
set -euo pipefail
...
echo "  Accept:      $ACCEPT"
echo "  Output:      $OUTPUT"
[[ -n "$SAVE_OUTPUTS" ]] && echo "  Save outputs: $SAVE_OUTPUTS"
echo "  Start at:    $START"
...
for idx in "${!QUERY_FILES[@]}"; do
    ...
    if [[ "$warmup_code" == "000" ]]; then
        to_ms=$(awk -v t="$TIMEOUT" 'BEGIN { printf "%.3f", t * 1000 }')
        for ((r = 1; r <= RUNS; r++)); do
    ...
    for ((r = 1; r <= RUNS; r++)); do
        ...
        # %{time_total} is fractional seconds; converted to fractional ms (3dp)
        # so sub-millisecond queries are not inflated by integer truncation.
        ...
        elapsed_int=$(printf "%.0f" "$elapsed_ms")
        ...
        if [[ -n "$SAVE_OUTPUTS" && "$http_code" == "200" ]]; then
            mkdir -p "$SAVE_OUTPUTS"
            cp "$tmpfile" "$SAVE_OUTPUTS/${query_id}-run${r}.tsv"
        fi
        ...
        last_error="$error"
        rm -f "$tmpfile"
        cumulative_ms=$(( cumulative_ms + elapsed_int ))
        # A timeout/error won't get better on re-run — stop this query here.
        [[ "$http_code" != "200" ]] && break
    done

    # Compute median time over the runs actually executed (sort -g handles floats)
    n_times=${#times[@]}
    ...
    if [[ "$last_status" == "200" ]]; then
        printf "OK  %9s ms (median)  %s bytes\n" "$median" "$last_size"
        PASSED=$((PASSED + 1))
