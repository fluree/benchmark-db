...
def load_summary(path):
    ...
    with open(path) as f:
        for r in csv.DictReader(f, delimiter="\t"):
            # A non-200 status (timeout/error) is a failure, not a time — record
            # None so it shows as "—", is excluded from aggregates, and is not
            # counted as passed (engines like Oxigraph time many queries out).
            try:
...
def cm(s):  # correctness mark
    ...
    # (both defensible, Fluree spec-correct) · ✗ minor ±1-row difference
...
def main():
    ...
    # Penalized geo means per the SPARQLoscope paper (Bast et al., ISWC 2025) are
    # the official headline aggregate: a failed/timed-out query counts as P x the
    # timeout, so the query set is identical across engines. Passed-only rows are
    # computed over each engine's own pass set, which flatters engines that time
    # out on the hard queries. An engine with zero failures has penalized ==
    # passed-only geo.
    timeout_ms = float(meta.get("methodology", {}).get("timeout_s", 300)) * 1000
    def penalized_geo(p):
        return lambda v: geomean([x if x is not None else p * timeout_ms for x in v])
    agg_row("geo mean (P=2)", penalized_geo(2))
    agg_row("geo mean (P=10)", penalized_geo(10))
    agg_row("geo mean (passed only)", geomean)
    agg_row("arith mean (passed only)", lambda v: (sum(x for x in v if x is not None)/len([x for x in v if x is not None])) if any(x is not None for x in v) else None)
    agg_row("median (passed only)", median)
    L.append("\n_geo mean (P=2/P=10) is the SPARQLoscope paper's official aggregate: a failed"
             f" or timed-out query counts as 2× / 10× the {int(timeout_ms/1000)} s timeout."
             " The passed-only rows average each engine's completed queries only, so they"
             " flatter engines with many failures._")
    ...
    if corr:
        ...
        L.append(f"Result-equivalence vs the reference engine (per-query `results` column above): "
                 ...
                 f"**{c.get('DIFF',0)} with minor ±1-row differences ✗**.")
        ...
        if diffs:
            L.append(f"\n**Minor ±1-row differences (✗)** — boundary/edge differences in regex-prefix and "
                     f"transitive-path handling; all other queries agree: "
